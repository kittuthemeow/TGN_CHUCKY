...
from TGNRobot import pbot as LYCIA
...
@LYCIA.on_message(filters.command("vc"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("chucky voice chat bot 😎😎😎")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("chucky the mass😎😎...")
    try:
        ...
        await m.edit(str(e))
        return
    await m.edit("wait pannunga nan audio record panitu eruka...")
    LyciaVoice = await ai_lycia(VoiceAi)
    await m.edit("connecting to chucky server...")
